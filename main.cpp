//============================================================
// STUDENT NAME: Tan Li En, Ryan
// NUS User ID.: E0543529
// COMMENTS TO GRADER: I tried very hard for this lab halp :<
// Hope you like the minecraft dirt block! and the sword on top of it! :)) Excaliburrr in minecraft haha
// ============================================================
...
const char cubeTexFile[] = "images/dirt.png";
const char metalTexFile[] = "images/metal2.png";
const char metal1TexFile[] = "images/metal3.png";
const char leatherTexFile[] = "images/leather.png";
...
void MakeReflectionImage( void )
{
    ...
    //****************************
    //Clearing the correct buffers
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    //Sets up the correct view volume for the imaginary viewpoint
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glFrustum(TABLETOP_Y1 - eyePos[1], TABLETOP_Y2 - eyePos[1], TABLETOP_X1 - eyePos[0], TABLETOP_X2 - eyePos[0], eyePos[2] - TABLETOP_Z, 5/3 * 10 * ROOM_WIDTH);

    //Sets up the imaginary viewpoint, a reflection of the original point in the x-y plane
    ...
    //Sets up the light source positions in the world space.
    glLightfv(GL_LIGHT0, GL_POSITION, light0Position);
    glLightfv(GL_LIGHT1, GL_POSITION, light1Position);

    //Draw the scene (but without the table)
    DrawRoom();
    DrawTeapot();
    DrawSphere();
    DrawCube();
    ...
    //Read the correct color buffer into the correct texture object
...
// This texture object is for the additional cube.
...
// This texture object is for the additional sword (metal parts)
    ...
    if (ReadImageFile(metalTexFile, &imageData,
        &imageWidth, &imageHeight, &numComponents) == 0) exit(1);
    if (numComponents != 3)
    {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
// This texture object is for the additional sword (metal1 parts)
    glGenTextures(1, &metal1TexObj);
    glBindTexture(GL_TEXTURE_2D, metal1TexObj);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);

    if (ReadImageFile(metal1TexFile, &imageData,
        &imageWidth, &imageHeight, &numComponents) == 0) exit(1);
    if (numComponents != 3)
    {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
// This texture object is for the additional sword (leather parts)
    glGenTextures(1, &leatherTexObj);
    glBindTexture(GL_TEXTURE_2D, leatherTexObj);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);

    if (ReadImageFile(leatherTexFile, &imageData,
        &imageWidth, &imageHeight, &numComponents) == 0) exit(1);
    if (numComponents != 3)
    {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
// Tabletop.
    ...
    SubdivideAndDrawQuad(24, 24, 1.0, 1.0, TABLETOP_X2, TABLETOP_Y2, TABLETOP_Z,
...
//Draw a basic black cube at the origin of the world.
void DrawCube( void ) {
    ...
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, matSpecular);
    glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, matShininess);
    glBindTexture(GL_TEXTURE_2D, cubeTexObj); //Use the cubeTexObj

    glPushMatrix();
    glTranslated(0.0, 0.0, TABLETOP_Z);
    glScaled(0.5, 0.5, 0.5);
    glTranslated(-1.0, 1.5, 0.0);

    //Draw the face in the +x direction
    glBegin(GL_TRIANGLE_FAN);
    glNormal3f(1.0, 0.0, 0.0);
    glTexCoord2d(0.0, 1.0); glVertex3d(0.5, -0.5, 1.0);
    glTexCoord2d(0.0, 0.0); glVertex3d(0.5, -0.5, 0.0);
    glTexCoord2d(1.0, 0.0); glVertex3d(0.5, 0.5, 0.0);
    glTexCoord2d(1.0, 0.0); glVertex3d(0.5, 0.5, 0.0);
    glTexCoord2d(1.0, 1.0); glVertex3d(0.5, 0.5, 1.0);
    glEnd();
    ...
    //Draw the face in the +y direction
    glBegin(GL_TRIANGLE_FAN);
    ...
    //Draw the face in the +z direction
    glBegin(GL_TRIANGLE_FAN);
    glNormal3f(0.0, 0.0, 1.0);
    glTexCoord2d(0.0, 1.0); glVertex3d(-0.5, -0.5, 1.0);
    glTexCoord2d(0.0, 0.0); glVertex3d(0.5, -0.5, 1.0);
    glTexCoord2d(1.0, 0.0); glVertex3d(0.5, 0.5, 1.0);
    glTexCoord2d(1.0, 0.0); glVertex3d(0.5, 0.5, 1.0);
    glTexCoord2d(1.0, 1.0); glVertex3d(-0.5, 0.5, 1.0);
    glEnd();
...
void DrawSword(void) {
    ...
    //Vertices of a 2D cube centered at the origin, lying on the y axis, in order top-left, bottom left, bottom right, top right
    double squareVertices[4][3] =
    {
        {0.0, -0.5, 1.0},
        {0.0, -0.5, 0.0},
        {0.0, 0.5, 0.0},
    ...
    //Translate to centre of cube
    glTranslated(-0.5, 0.75, 0.5 + TABLETOP_Z);
    //Rotate about origin
    glRotated(-45, 0.0, 0.0, 1.0);
    //Translate bottom of handguard to origin
    glTranslated(0.05, -1.0, -0.21 + 0.05);

    //Draw top of sword
    glPushMatrix();
    glTranslated(0.0, 0.0, 0.25);
    glRotated(-90.0, 0.0, 1.0, 0.0);
    glScaled(1.0, 2.0, 0.10);
    glNormal3f(1.0, 0.0, 0.0);
    SubdivideAndDrawQuad(24, 24,
        0.0, 1.0, squareVertices[0][0], squareVertices[0][1], squareVertices[0][2],
        0.0, 0.0, squareVertices[1][0], squareVertices[1][1], squareVertices[1][2],
        1.0, 0.0, squareVertices[2][0], squareVertices[2][1], squareVertices[2][2],
        1.0, 1.0, squareVertices[3][0], squareVertices[3][1], squareVertices[3][2]);
    glPopMatrix();

    //Draw left top angled surface of sword, facing +x
    glPushMatrix();
    glTranslated(0.03, 0.0, 0.21);
    glRotated(-45.0, 0.0, 1.0, 0.0);
    glScaled(1.0, 2.0, 0.05);
    glNormal3f(1.0, 0.0, 0.0);
    SubdivideAndDrawQuad(24, 24,
        0.0, 1.0, squareVertices[0][0], squareVertices[0][1], squareVertices[0][2],
        0.0, 0.0, squareVertices[1][0], squareVertices[1][1], squareVertices[1][2],
    ...
    //Draw right top angled surface of sword, facing -x
    glPushMatrix();
    glFrontFace(GL_CW);
    glTranslated(-0.13, 0.0, 0.21);
    glRotated(45.0, 0.0, 1.0, 0.0);
    glScaled(1.0, 2.0, 0.05);
    glNormal3f(1.0, 0.0, 0.0);
    SubdivideAndDrawQuad(24, 24,
    ...
    //Draw right bottom angled surface of sword, facing +x
    glPushMatrix();
    ...
    glVertex3d(-0.05, -1.20, 0.21);
    glTexCoord2d(0.0, 0.5);
    glVertex3d(0.0, -1.0, 0.25);
    glTexCoord2d(1.0, 0.0);
    glNormal3f(0.0, -1.0, 1.0);
    glVertex3d(-0.10, -1.0, 0.25);
    glTexCoord2d(1.0, 0.0);
    glNormal3f(-1.0, -1.0, 1.0);
    glVertex3d(-0.13, -1.0, 0.21);
    glTexCoord2d(1.0, 0.0);
    glNormal3f(-1.0, -1.0, -1.0);
    glVertex3d(-0.10, -1.0, 0.17);
    glTexCoord2d(1.0, 0.0);
    glNormal3f(0.0, -1.0, -1.0);
    glVertex3d(0.0, -1.0, 0.17);
    glTexCoord2d(1.0, 0.0);
    glNormal3f(1.0, -1.0, -1.0);
    glVertex3d(0.03, -1.0, 0.21);
    glTexCoord2d(1.0, 0.0);
    glNormal3f(1.0, -1.0, 1.0);
    glVertex3d(0.0, -1.0, 0.25);
    glEnd();

    //Draw handguard of sword
    glPushMatrix();
    glTranslated(-0.05, 1.00, 0.21);
    glScaled(1.0, 0.10, 0.10);
    glTranslated(0.0, 0.0, -0.5);
    glBindTexture(GL_TEXTURE_2D, metal1TexObj);
    ...
    glBindTexture(GL_TEXTURE_2D, leatherTexObj);
    glPushMatrix();
    glTranslated(-0.05, 1.50, 0.21);
    glRotated(90.0, 0.0, 0.0, 1.0);
    glScaled(1.0, 0.075, 0.075);
    glTranslated(0.0, 0.0, -0.5);
